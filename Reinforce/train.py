...
def run_evaluate_episode(agent, env, render=False):
    eval_reward = []
    for i in range(5):
        obs = env.reset()
        episode_reward = 0
...
def train():
    ...
    for episode in range(1000):
        ...
        if episode % 10 == 0:
            logger.info("Episode {}, Reward sum {}".format(episode, sum(reward_list)))
        ...
        if (episode + 1) % 5 == 0:
            # render = True
            total_reward = run_evaluate_episode(agent, env, render=False)
